...
| R1-T0-START | T0 | T0-15a start() on P16 | RET_EQ_COG_M1 | 0 | 7 | COGID | 1 | - | D | 1 | OWED | - | start() returns motorCog-1 and testGetMotorCog() returns motorCog (isp_bldc_motor.spin2:115-116, :1208); the return must be in 0..7 and equal testGetMotorCog()-1 (design A.4 row 1) |
| R1-T0-EXHAUST | T0 | T0-15b cog exhaustion, runs last | RET_NEG_NOLEAK | -1 | -1 | COGID | 1 | - | D | 1 | OWED | - | isp_bldc_motor.spin2:110-113; the binary also requires testGetMotorCog()==0 and free cogs after release == baseline (design A.5-a) |
| R1-T0-RESTART | T0 | T0-15c start() twice with no stop() | NO_ORPHAN | 0 | 0 | COUNT | 1 | - | D | 1 | OWED | - | PL-24 fix isp_bldc_motor.spin2:103-104; measured = abs(free2-free1) + abs(free3-baseline), and the binary also requires free1 == baseline-1 (design A.4 row 1, A.5-a) |
...
| R2-HOST-DETDIFF | HOST | detect-phase2 log diffed against the 2026-09-11 baseline and the 3505 prediction list (design C.2); inputs: file:DOCs/analyses/bench/SIGNOFF-DETECT-PREDICTIONS.tsv file:DOCs/analyses/bench/2026-09-11/debug_260911-210229.log src:src/test_bench_detect.spin2:phase2_compiled | ONLY_PREDICTED | 0 | 0 | COUNT | 1 | - | D/M | 1 | OWED | - | measured = unpredicted changes + predicted changes that did not occur; baseline 2026-09-11/debug_260911-210229.log [M]; sum tolerance +-11 from the measured Rev B band 93-104 (isp_bldc_motor.spin2:851-853) [D]; design C.2 |
...
| R2-DETECT-GUARD | HOST | detect-phase2 log shows the gate-input guard held (design C.3 item 5); inputs: src:src/test_bench_detect.spin2:GATE_OVERLAP file:DOCs/analyses/bench/SIGNOFF-DETECT-PREDICTIONS.tsv | GUARD_SKIPS_OK | TRUE | TRUE | BOOL | 1 | - | D | 1 | OWED | - | TRUE when no BD-REP exists for a skipped group, BD-MAP classifies those groups as gate overlaps, and every BD-SWEEP count equals the plan less the skipped cells (design C.3 item 5); it can FAIL: today's detect binary emits BD-REP for P40_P55 |
...
| cell | bin | test | crit | lo | hi | units | min_inst | count_filter | prov | visit_owed | status | status_ref | basis |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R4-T0-1M-TICKS | T0 | T0-3 stopAfterDistance() tick targets | DDU_M_TICKS | 173 | 174 | TICKS | 1 | - | D | 1 | OWED | - | 1000 mm x 90 / 518.6 mm = 173.5, truncated or rounded (design A.5-b); the pre-fix 175 [M debug_260911-143911.log] and finding F's 17 both FAIL; the binary also FAILs when the three units disagree |
...
| cell | bin | test | crit | lo | hi | units | min_inst | count_filter | prov | visit_owed | status | status_ref | basis |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R9-SCAN-HALFLEG | SCAN | each half-speed leg | FIT_OR_CLIFF | TRUE | TRUE | BOOL | 4 | - | M | 1 | OWED | - | in run 5 both left half legs ended TOO_FEW with no probe between the last good point and the fault (evaluation section 3); v4 CHANGE 4 must end each leg OK/POOR or with BS-CLIFF found TRUE; a skipped leg is NOMEAS |
| R9-SCAN-OWNZERO | SCAN | each fitted leg | PTS_NO_OWN_ZERO | 0 | 0 | COUNT | 4 | - | D | 1 | OWED | - | v4 CHANGE 2 invariant: every point that feeds a fit carries its own lifetime zero (bPtZeroValid); run 5's ratio 0.440 vs 1.16 came from violating it [M section 6] |
...
| cell | bin | test | crit | lo | hi | units | min_inst | count_filter | prov | visit_owed | status | status_ref | basis |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R10-SCAN-RSTALONE | SCAN | first natural fault per motor (J.1 ruled option b) | RESET_ALONE_CLEARS | 0 | 2030 | MS | 1 | - | D | 1 | OWED | - | PL-28 item 1: before the fix testResetFault() with an unchanged command waits 30 ms + 1000 x 2 ms = 2030 ms and stays FAULTED (isp_bldc_motor.spin2:1114-1132); measured = ms to cleared, FAIL if not cleared |
...
| R11-WDT-FIRES | SCANWD | watchdog cog, -D WD_SELFTEST build | STALL_MS | 4000 | 4500 | MS | 1 | - | D | 1 | OWED | - | a stall is declared at >= WD_STALL_MS 4000 by construction; hi = WD_STALL_MS + 2 x WD_POLL_MS = 4500 (design G.4) |
| R11-WDT-CKPT | SCANWD | watchdog cog, -D WD_SELFTEST build | CKPT_IS_SELFTEST | TRUE | TRUE | BOOL | 1 | - | D | 1 | OWED | - | the stall is placed at CP_SELFTEST; any other checkpoint means the heartbeat contract misattributes (design G.4) |
| R11-WDT-STOPPED | SCANWD | watchdog cog, -D WD_SELFTEST build | WHEELS_STOPPED | TRUE | TRUE | BOOL | 1 | - | D | 1 | OWED | - | at least one driver cog running before the stall and both testGetMotorCog()==0 after the watchdog's stop()s (design G.4) |
| R11-WDT-STACK | SCANWD | watchdog cog, -D WD_SELFTEST build | STACK_OK | TRUE | TRUE | BOOL | 1 | - | D | 1 | OWED | - | WD_STACK_SENTINEL intact (test_bench_scan.spin2:4375; design G.4) |
